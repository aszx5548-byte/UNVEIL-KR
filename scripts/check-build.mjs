...
const FORBIDDEN = [
  ...
  // 쉼표 없는 "19900원" 과 "1만 9천원" 도 잡습니다. 쇼핑몰 표기로는 이쪽이 더 흔한데
  // 전에는 쉼표가 있는 형태만 봤습니다.
  ['가격 표기', /(?:\d{1,3}(?:,\d{3})+|\d{4,7})\s*원|₩\s?\d|\d+\s*만\s*\d*\s*천?\s*원/],
  // 화면에 안 보여도 소스에 남은 가격은 잡습니다. 소스를 열면 그대로 읽히고,
  // 8/23 앱 교체 때 실제로 unveilPrice:19900 이 이렇게 다시 들어왔습니다.
  ['소스에 남은 가격', /\b\w*(?:price|cost|amount|krw|won)\s*[:=]\s*["']?\d{3,}/i],
  // 한글 표기를 놓쳤습니다. "8밀리" 가 실제로 배포본까지 나갔습니다.
  // 한글 표기 뒤에는 \b 를 쓰면 안 됩니다. 한글은 \w 가 아니라서 경계가 생기지 않고,
  // 그 때문에 "8밀리" 가 검사를 그대로 통과해 배포본까지 나갔습니다.
  // 앞의 (?<!\d) 는 "18밀리" 안의 8 을 잡지 않기 위한 것입니다.
  ['규격 8mm', /\b8(?:\.0)?\s?mm\b|(?<!\d)8\s?(?:밀리|미리|파이)/i],
  // 조달·원가 판단은 상품 페이지 소스에 남으면 안 됩니다.
  // 실제로 "단가가 높아", "조달처 …가 염색으로 확인되어" 가 나가 있었습니다.
  // 앞에 한글이 붙은 "결단가"(결단+가) 같은 말까지 잡히지 않게 막습니다.
  ['내부 메모', /(?<![가-힣])(?:단가|매입가|마진)|조달처|진위 불확실|재구성품|염색 하울라이트/],
...
function walk(dir) {
  for (const name of readdirSync(dir)) {
    ...
    // 남이 만든 압축 번들은 건너뜁니다. base64 덩어리 안에 "8MM" 같은 조각이
    // 우연히 들어 있어, 우리 잘못이 아닌 것으로 빌드가 멈출 수 있습니다.
    if (/\.min\.js$/i.test(name)) continue;
